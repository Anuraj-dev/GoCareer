...
router.get('/career-roadmap/:title', (req, res) => {
  ...
  try {
    ...
    if (req.session.careerPaths) {
      careerDetails = req.session.careerPaths.find(c => 
        c.title.toLowerCase() === decodeURIComponent(careerTitle).toLowerCase());
    }
    
    if (!careerDetails) {
      req.app.locals.logger.warn(`[Routes] Career title ${careerTitle} not found in session data`);
    ...
    req.app.locals.logger.error(`[Routes] Error rendering career roadmap for title ${careerTitle}: ${error.message}`);
    res.render('error', {
      message: "Error loading career roadmap",
      error: { status: 500, details: req.app.get('env') === 'development' ? error.message : 'Internal server error' }
    });
  }
});
